 ...
 * the model with Oil Production and Oil Shock Effects.
 * --- ---- --- --- --- --- ---- --- --- ------ ---- --- --- --- --- ---
...
varexo  
at wt nt tct tkt pit twt it gt igt trt rt pixt pimt e_pistar e_ystar e_Rstar phitildet dt oilt;// 
...
parameters  rho_p fc omega_w chi_w e_nu chi_h sigma_c  piT chi_x chi_im
            ...
            rho_tc rho_tk rho_tw rho_g rho_ig rho_tr rho_oil oil_bar ela_oil oilg_bar
            phimc phimi pcd_bar pim_bar phitilde phitildes rho_phitilde b_target 
            rho_pistar rho_ystar rho_Rstar piTstar cm_bar im_bar x_bar //
            ;
...
// Oil block
rho_oil      =     0.70   ;   
oil_bar      =     0.072  ;  // share of oil product. as percent. of GDP 
oilg_bar     =     0.015  ;  // share of oil gov. revenues as percent. of GDP 
...
rho_R        =     0.87  ;  // (fixed following to Christiano et al. (2010)
r_pi         =     1.5685  ;
r_y          =     0.1485  ;
...
x_bar    = cm_bar+im_bar;
tr_bar   = tao_cbar*c_bar+tao_wbar*wN_bar+tao_kbar*rky_bar-g_bar-Ig_bar-R_bar*b_bar+b_bar+oilg_bar;
...
// 11. Production
y_hat=fc*(e_a+alfa*k_hat(-1)+alfa*u_hat+(1-alfa)*N_hat+alfa_g*kg_hat(-1))+oil_bar*oil_hat; 
...
y_hat=(c_bar)*c_hat+(I_bar)*I_hat+(1-tao_kbar)*(rky_bar)*u_hat+g_bar*g_hat+Ig_bar*Ig_hat+oil_bar*oil_hat; 

// 19. total consumption
c_hat=(1-omega_h)*cr_hat+omega_h*cnr_hat; 

// 20-21. monetary policy rule:
R_hat = rho_R*R_hat(-1) + (1-rho_R)*(r_pi*(piT+(pid_hat(-1)-piT)) + r_y*(y_hat(-1)))+ rt; //
...
g_hat*g_bar+Ig_bar*Ig_hat+tr_bar*trans+b_bar*R_bar*(b_hat(-1)+R_hat(-1)-pid_hat)=b_bar*b_hat+tao_cbar*c_bar*(c_hat+tao_c)+
                            tao_wbar*wN_bar*(w_hat+N_hat+tao_w)+tao_kbar*rky_bar*(rk_hat+u_hat+k_hat(-1)+tao_k)+ oilg_bar*(oil_hat+oil_bar); // 
...
// Oil block
oil_hat = rho_oil * oil_hat(-1) + oilt;
...
varobs 
data_y data_pi data_r data_con data_gov data_inv data_pistar data_rstar data_ystar data_pet; 
...
// Schock simul..
stoch_simul (order=2,nofunctions,irf=40, nocorr, noprint, nograph);

// Simul
%simul(periods=40,stack_solve_algo=0); %stack_solve_algo=0
